...
| R5 | **Fault 3 "Unknown Failure" 与 Runbook 的关系未设计。** §15.3 的 patch 加的是 "verify upstream"，但 §15.2 Fault 2 已经是 upstream mismatch。若 v0.1 已能处理 F2，则 F3 演示不成立。且 v0.1 "必然失败" 不能靠模型随机性。 | 这是整个 demo 最关键 30 秒，必须可复现。 | 重新设计 fault 与 runbook 的对应关系（见 §0.3）。核心思路：**知识在文档里但没被编译进 v0.1**（Appendix 段落触发词不匹配），Failure Analyzer 通过 FTS 检索找回证据 → 生成 patch。这是一个诚实且确定性的故事。 |
...
Runbook 的写法要保证：Appendix B 在 Knowledge Extractor 中被抽为独立的 `diagnostic_rule` 类型 KU，`trigger` 是 "nginx reload failed / upstream mismatch"；Compiler pass 2 按 SkillSpec triggers 筛 KU 时不会选中它；Failure Analyzer 用 `"502 upstream nginx -t"` 做 FTS 检索时能召回它。这三点各有一个测试锁定（C5.8、C6.9、C7.8）。
...
│   ├── knowledge/               # FTS 检索、KnowledgeExtractor
...
| # | Commit | 内容 | 完成标准 |
|---|---|---|---|
| C2.1 | `feat(db): sqlite schema and connection` | `db/schema.sql`：§9 全部表 + `chunks` + FTS5 虚拟表；`db/connection.py`（WAL、外键）；`db/init.py` 幂等建表 | 空库初始化两次无报错 |
...
| C5.3 | `feat(ingestion): chunking with source locations` | 标题感知切块；每块记 `page/line_start/line_end`；写 `chunks` 表 | 单测：行号可回溯 |
| C5.4 | `feat(knowledge): FTS5 search over chunks and knowledge units` | `search(query, project_id, type=None, k=10)`；BM25 排序 | 单测：`"nginx -t upstream"` 能召回 Appendix B |
| C5.5 | `feat(knowledge): LLM knowledge extractor` | 每 chunk → structured output → `KnowledgeUnit[]`（§8.2 类型枚举、`source_refs`、`confidence`）；按 title 去重合并 | 单测 FakeModel；golden 期望 KU 集合快照 |
| C5.6 | `feat(api): sources upload/list, extract, knowledge units` | `POST/GET /api/projects/:id/sources`、`POST /api/projects/:id/extract`、`GET /api/projects/:id/knowledge` | API 测试（multipart 上传） |
| C5.7 | `feat(demo): service recovery runbook (md + pdf)` | `demo/docs/service-recovery-runbook.md` 按 §0.3 设计：主章节 Service Down / 502；Appendix B 独立触发词；导出 PDF | 人工检查 + C5.8 锁定 |
| C5.8 | `test(knowledge): runbook extraction yields Appendix B as diagnostic_rule` | 端到端（FakeModel 或录制响应）：Appendix B 成为独立 KU，type=`diagnostic_rule`，FTS 可召回 | 测试通过 |
...
| C6.2 | `feat(compiler): pass 1-3 knowledge → capability boundary → SkillSpec` | 按 task scope 选 KU（trigger 匹配）；LLM 生成 spec 草稿；规则层收紧 permissions（永不放宽） | FakeModel 单测；F3 的 Appendix B **不被选入**（锁定 §0.3） |
...
| C7.2 | `feat(evolution): evidence retrieval for failure` | 用 symptom/failed_assertion 做 FTS → `source_support`（doc#page） | 单测：F3 召回 Appendix B |
...
| 4 | C4.4–C4.8、C5.1–C5.4 | Without vs With 数字出来 |
| 5 | C5.5–C5.8、C6.* | 上传 runbook → 自动编译出 skill（DoD 1–4） |
